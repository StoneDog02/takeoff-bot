...
describe("Beckstead opening identity (Amendment #3)", () => {
  it("resolves frozen Mode B evidence without sanitize-collision crash", () => {
    ...
    const categoryTrace = dining.resolutionTraces.find(
      (entry) => entry.propertyPath === "category",
    );
    assert.ok(
      categoryTrace?.explanation.includes("E-DOOR-3068-DINING"),
      "expected dining evidence cited in category trace",
    );
    assert.ok(
      categoryTrace?.explanation.includes("E-D3068-DOOR-DINING-DIM"),
      "expected dining dim evidence cited in category trace",
    );

...
import {
  ...
  writeResetTakeoff,
} from "../../../../src/scopes/framing/reset/writeResetTakeoff.js";
...
describe("reset takeoff output", () => {
  it("writes reset-takeoff.json with wall materials and no pendingClaims", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "reset-takeoff-"));
    try {
      ...
      const written = await writeResetTakeoff({
        projectId: "proj-reset-1",
        artifactsRoot: root,
        takeoff,
      });
      assert.equal(path.basename(written), RESET_TAKEOFF_FILENAME);
  ...
  it("orchestrator with constructionOverride writes takeoff without reader", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "reset-orch-"));
    try {
      const result = await runFramingResetTakeoff({
        projectId: "proj-reset-2",
        pdfPath: "/tmp/sample.pdf",
      ...
      assert.ok(result.takeoffPath?.endsWith(RESET_TAKEOFF_FILENAME));
...
describe("reset opening assumptions disclosure", () => {

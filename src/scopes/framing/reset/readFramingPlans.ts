...
/**
 * READ THE PLANS for the factory-reset path.
 *
  ...
  if (input.writeDebugArtifacts) {
    debugPaths.push(
      await writeDebugJson(
        artifactsRoot,
        input.projectId,
        "reset-page-classification.json",
        pageClassification,
      ),
    );
    debugPaths.push(
  ...
  if (input.writeDebugArtifacts) {
    debugPaths.push(
      await writeDebugJson(
        artifactsRoot,
        input.projectId,
        "reset-compiled-drawing-pages.json",
        { pages: compiledPages },
      ),
    );
  }

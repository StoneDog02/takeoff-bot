...
```
app.ts main()
  indexPlan(pdfPath)                           # src/plans/indexPlan.ts
  runFramingResetTakeoff(input)                # src/scopes/framing/reset/runFramingResetTakeoff.ts
    readFramingPlans(input)                    # src/scopes/framing/reset/readFramingPlans.ts
      ...
      interpretFramingConstruction(evidence)   # src/scopes/framing/reset/interpretFramingConstruction.ts
    ...
    calculateFramingTakeoff(construction)      # src/scopes/framing/reset/calculateFramingTakeoff.ts
    ...
    buildResetTakeoff(...)                     # src/scopes/framing/reset/writeResetTakeoff.ts
...
**Boundary:** `indexPlan` in `src/plans/indexPlan.ts`. Converts PDF to JSON page text via OpenDataLoader, enriches with outline bookmarks, computes content hash. Returns `PlanIndex`.
...
**Boundary:** `calculateFramingTakeoff` in `src/scopes/framing/reset/calculateFramingTakeoff.ts`. Takes `FramingConstruction`, returns `{ materials, assumptions }`.
...
**Boundary:** `buildResetTakeoff` + `writeResetTakeoff` in `src/scopes/framing/reset/writeResetTakeoff.ts`. Produces `reset-takeoff.json`.
...
`src/scopes/framing/reset/framingConstruction.schema.ts` -- a Zod object composing six domain payload schemas.
...
All 6 leaf adapt functions are in `src/scopes/framing/reset/`:
...
**None of the interpret functions perform construction discovery (A) or deterministic derivation (B).** Every one is a single-line delegation to a `resolve*` function in `src/scopes/framing/resolvers/`. The resolvers themselves do legitimate work (subject grouping, candidate selection, property resolution). The interpret layer adds zero value.
...
All within `readFramingPlans` (`src/scopes/framing/reset/readFramingPlans.ts`):
...
The assumption registry is at `src/scopes/framing/claims/assumptionRegistry.ts`. Three opening defaults:
...
Assumption placement is **acceptable** and does not violate the target architecture. The assumption registry's location in `src/scopes/framing/claims/` is a naming/organizational artifact (it shares a directory with old claim machinery) but is not functionally entangled.
...
Schema: `src/scopes/framing/reset/resetTakeoff.schema.ts`
Writer: `buildResetTakeoff` + `writeResetTakeoff` in `src/scopes/framing/reset/writeResetTakeoff.ts`
...
`src/app.ts:39` -- when passed, routes to `PipelineRunner` with the old 16-stage `framingScope.stages` from `src/scopes/framing/stages/createFramingStages.ts`.
...
|---|---|---|
| `src/scopes/framing/stages/createFramingStages.ts` | Legacy 16-stage definition | **Safe deletion candidate** (after escape hatch removal) |
| `src/core/pipeline/PipelineRunner.ts` | Generic pipeline runner | **Safe deletion candidate** |
| `src/scopes/framing/calculators/calculation-coordinator.ts` | Legacy calculator coordinator (adds fasteners, pendingClaims) | **Safe deletion candidate** |
| `src/scopes/framing/calculators/calculateFasteners.ts` | Fastener calculator (not called on reset path) | **Harmless unreachable legacy** |
| `src/scopes/framing/validators/validation-coordinator.ts` | Stage 13 validation | **Safe deletion candidate** |
| `src/scopes/framing/confidence/` (entire directory) | Stage 15 confidence evaluation | **Safe deletion candidate** |
| `src/scopes/framing/observability/` (entire directory) | Stage 16 product state / readiness | **Safe deletion candidate** |
| `src/scopes/framing/claims/collectPendingClaims.ts` | PendingClaim lifecycle | **Safe deletion candidate** |
| `src/scopes/framing/claims/buildClaimCandidacyContext.ts` | Claim candidacy context | **Safe deletion candidate** |
| `src/scopes/framing/claims/admitMaterialClaimCandidate.ts` | Claim admission | **Safe deletion candidate** |
| `src/scopes/framing/review-workspace/` | Review workspace | **Harmless unreachable legacy** |
| `src/scopes/framing/audit/` | Audit metrics | **Useful debug tool** |
| `src/scopes/framing/resolvers/applyWallOpeningBacklinks.ts` | Backlink application | **Harmless unreachable legacy** |
| `src/scopes/framing/resolvers/linkOpeningHeaderRelationships.ts` | Opening-header linking | **Harmless unreachable legacy** |

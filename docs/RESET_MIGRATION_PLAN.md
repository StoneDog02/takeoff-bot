...
| Drawing Compiler (`src/drawing-compiler/**`) | Plan reading / geometry |
| Project Learning + ODL harvest (`src/project-interpreter/projectLearning/**`) | Plan reading / project defs |
...
|---------|-----------------|--------------------------------|
| `src/scopes/framing/claims/assumptionRegistry.ts` | Closed Brain-authorized defaults; consult-before-block | Standalone assumptions module (e.g. `…/assumptions/`); called **from each domain calculator** |
...
See each phase in §3. Baseline: existing `tests/scopes/framing/*`, `tests/core/*calculator*`, user-decision Run-2 tests, Beckstead artifact replay under `artifacts/beckstead-floor-wb-restore-20260829-122807/`.
...
- `tests/scopes/framing/material-claim-authority-ladder.test.ts` → rewrite to assumption-registry tests (no claim ladder).  
...
Evidence sources: `artifacts/architecture-audit/RESET_ARCHITECTURE_AUDIT.md`, locked decisions in the authorizing prompt, and current code under `src/scopes/framing/**`, `docs/FRAMING_V1_LIMITATIONS.md`, Beckstead artifacts `beckstead-floor-wb-restore-20260829-122807`.

...
|-----------|------|-----------|-------|----------------------|-----------------|---------------------|
| `indexPlan` / PlanIndex | [`src/plans/indexPlan.ts`](src/plans/indexPlan.ts) | PDF → page index | **A** | Yes | Yes | Keep as upload entry |
| Stage 1 `verifiedPlanSet` | [`createFramingStages.ts`](src/scopes/framing/stages/createFramingStages.ts) | Persist plan index as stage | **E/F** | No as production stage (D1) | Plan index concept | Dev artifact optional; not a stage |
| Page classification | [`resolvePageClassificationForPipeline.ts`](src/plans/resolvePageClassificationForPipeline.ts), [`classifyPlanPages.ts`](src/plans/classifyPlanPages.ts) | Page kinds/roles | **B** | Not as own stage (D2/D5) | Yes | Internal to `readPlans` |
| Reading order | [`buildPlanReadingOrder.ts`](src/plans/buildPlanReadingOrder.ts) | Ordered pages | **B** | Not as own stage (D3) | Yes | Absorb into `readPlans` |
| Stage 4 buildingAssemblies | stub in `createFramingStages` | Hardcoded mock assemblies | **F** | No (D4) | No | Remove; stop feeding Claude |
| Project Learning | [`projectLearning/runProjectLearning.ts`](src/project-interpreter/projectLearning/runProjectLearning.ts) | Project defs from schedules/notes | **B** | Capability yes; stage no (D6) | Yes | Inside `readPlans` |
| Project Orientation / dictionary | [`buildOrientationDictionary.ts`](src/project-interpreter/buildOrientationDictionary.ts) | Project conventions | **B** | Impl not locked (D7) | Yes | Inside `readPlans` |
| `DictionaryGovernor.govern` | [`dictionaryGovernor.ts`](src/project-interpreter/dictionaryGovernor.ts) | Reader-integrity of defs | **B** | Abstraction not locked (D8) | Integrity checks yes | Inside `readPlans`; simplify later OK |
| Drawing Compiler | [`compileDrawingPage.ts`](src/drawing-compiler/compileDrawingPage.ts) | Geometry, marks, dims, schedules | **B** | Yes capability (D9) | Yes | Inside `readPlans`; audits = **E** |
| Claude extraction | [`runFramingExtractionPasses.ts`](src/scopes/framing/extraction/runFramingExtractionPasses.ts), [`extractFramingEvidence.ts`](src/scopes/framing/prompts/extractFramingEvidence.ts) | Plan reading | **B** | Prompt/pass sequencing yes | Yes | Inside `readPlans` |
| Geometry/semantic Evidence bridges | [`src/scopes/framing/geometry/*`](src/scopes/framing/geometry) | Compiler → Evidence rows | **B→C adapter / F packaging** | Evidence packaging **F** as authority (D10) | Useful facts yes | Absorb into construction adapters; keep bridges only while adapters need them |
...
| `resolveWallFraming` | [`resolveWallFraming.ts`](src/scopes/framing/resolvers/resolveWallFraming.ts) | Evidence → walls | **F stage**; buried merge **B/C** | Stage no (D13) | Type/schedule merge yes | → `interpretWalls` construction adapter |
| `resolveOpenings` | [`resolveOpenings.ts`](src/scopes/framing/resolvers/resolveOpenings.ts) | Evidence → openings + ObjectId parents | **F stage** (D15) | Opening objects yes | Host-wall context yes | → `interpretOpenings`; attach host assembly, not ObjectId chain as authority |
| `applyWallOpeningBacklinks` | [`applyWallOpeningBacklinks.ts`](src/scopes/framing/resolvers/applyWallOpeningBacklinks.ts) | Reverse `openingIds` | **F/E** | Not required (D15) | Optional debug only | Drop from calc path |
| `applyStructuralMemberAuthority` | [`structuralMemberAuthority.ts`](src/scopes/framing/resolvers/structuralMemberAuthority.ts) | Dimensional/mark/qty/synonym rules | **C** (absorb) | Authority subsystem **F** (D16) | Rules yes | → structural interpret/calc prep |
| `linkOpeningHeaderRelationships` | [`linkOpeningHeaderRelationships.ts`](src/scopes/framing/resolvers/linkOpeningHeaderRelationships.ts) | Tag → ObjectId links | **F** as gate (D16) | Reader relationship meaning **B** | Keep mark associations if reader has them | Not calc gate |
| Floor helpers | [`floorFragmentConsolidation.ts`](src/scopes/framing/resolvers/floorFragmentConsolidation.ts), [`floorLayoutAuthority.ts`](src/scopes/framing/resolvers/floorLayoutAuthority.ts), [`floorAreaMaterialCompatibility.ts`](src/scopes/framing/resolvers/floorAreaMaterialCompatibility.ts) | Joist parse, MAX SPAN, fragments, spacing axis, slab | **B/C** | Resolver stage **F** (D18) | All listed rules yes | → floor interpret before calc |
| Roof resolve | [`resolveRoofFraming.ts`](src/scopes/framing/resolvers/resolveRoofFraming.ts) | Evidence → roof objects | **F stage** (D19) | Plain roof structs OK | Family from reader + `isStickCommonRafterFramingType` | Family gate stays in calculator |
| Sheathing resolve | [`resolveSheathing.ts`](src/scopes/framing/resolvers/resolveSheathing.ts) | Evidence → system/area + backlinks | **F stage** (D17) | Spec+coverage concepts yes | Application canonicalize yes | → sheathing interpret; no parentSystemId gate |
| Domain calculators | [`calculators/calculate*.ts`](src/scopes/framing/calculators) | Formulas | **C** | Orchestration contract no (D21) | Formulas + local guards yes | Strip validation/claim deps |
| Opening assumption factories + registry | [`createOpening*Assumption.ts`](src/scopes/framing/calculators), [`assumptionRegistry.ts`](src/scopes/framing/claims/assumptionRegistry.ts) | Three governed defaults | **C** | Registry API not locked (D21) | Three behaviors yes | Keep reachable from opening calc; leave registry in place first reset |
| `coordinateFramingCalculations` | [`calculation-coordinator.ts`](src/scopes/framing/calculators/calculation-coordinator.ts) | Fan-out + pendingClaims | **C** simplify | Pending path **F** (D22) | Fan-out yes | New thin coordinator without claims/validation |
| Stage 13 validation | [`validation-coordinator.ts`](src/scopes/framing/validators/validation-coordinator.ts) | Material permission | **F** (D20) | No centralized gate | Local calc guards already exist | Delete from production path |
| `isQuantityBlocked` | [`isQuantityBlocked.ts`](src/scopes/framing/calculators/isQuantityBlocked.ts) | Stage 13 suppress | **F** | No | — | Remove from calculators |
| Claims / PendingMaterialClaim | [`src/scopes/framing/claims/*`](src/scopes/framing/claims) | Candidacy/pending | **F** (D22) | Assumption consult only | Assumption consult only | Delete pending/candidacy from production |
| Stage 15 confidence | [`confidence/*`](src/scopes/framing/confidence) | Labels after calc | **F** (D23) | No | — | Remove from production |
...
| Orchestration | New [`src/scopes/framing/reset/runFramingResetTakeoff.ts`](src/scopes/framing/reset/runFramingResetTakeoff.ts) |
| Reader | New [`src/scopes/framing/reset/readFramingPlans.ts`](src/scopes/framing/reset/readFramingPlans.ts) composing existing Stage 2/3/5/6 functions |
| Reader output boundary | `FramingConstruction` schema module under `src/scopes/framing/reset/` (or `schemas/construction-understanding.schema.ts`) |
| Calculation | New thin [`src/scopes/framing/reset/calculateFramingTakeoff.ts`](src/scopes/framing/reset/calculateFramingTakeoff.ts) wrapping preserved calculators |
...
### Walls ([`calculateWallFraming.ts`](src/scopes/framing/calculators/calculateWallFraming.ts))
...
### Openings ([`calculateOpeningFraming.ts`](src/scopes/framing/calculators/calculateOpeningFraming.ts))
...
### Structural members ([`calculateStructuralMembers.ts`](src/scopes/framing/calculators/calculateStructuralMembers.ts))
...
### Sheathing ([`calculateSheathing.ts`](src/scopes/framing/calculators/calculateSheathing.ts))
...
### Floor ([`calculateFloorFraming.ts`](src/scopes/framing/calculators/calculateFloorFraming.ts))
...
### Roof ([`calculateRoofFraming.ts`](src/scopes/framing/calculators/calculateRoofFraming.ts))
...
| Item | Detail |
|------|--------|
| Where | Factories under `calculators/createOpening*.ts`; consult via [`assumptionRegistry.ts`](src/scopes/framing/claims/assumptionRegistry.ts); applied inside [`calculateOpeningFraming.ts`](src/scopes/framing/calculators/calculateOpeningFraming.ts) |
...
- **Create:** `src/scopes/framing/reset/*` (`runFramingResetTakeoff`, `readFramingPlans` stub, `FramingConstruction` schema, `writeResetTakeoff`, `calculateFramingTakeoff` stub).
...
| `src/scopes/framing/reset/runFramingResetTakeoff.ts` | — | **CREATE** | orchestrator | 1 | Entry |
| `src/scopes/framing/reset/readFramingPlans.ts` | — | **CREATE** | B | 1–2 | Compose reader |
| `src/scopes/framing/reset/framingConstruction.schema.ts` | — | **CREATE** | B→C boundary | 1 | Plain structs |
| `src/scopes/framing/reset/interpret*.ts` | — | **CREATE** | B/C | 3 | From resolvers |
| `src/scopes/framing/reset/calculateFramingTakeoff.ts` | — | **CREATE** | C | 1/3 | Thin fan-out |
| `src/scopes/framing/reset/writeResetTakeoff.ts` | — | **CREATE** | D temp | 1 | Temp JSON |
| `src/app.ts` | 16-stage runner | **MODIFY** | A entry | 1 | Call reset |
| `src/scopes/framing/stages/createFramingStages.ts` | 16 stages | **BYPASS** / later delete | E audit only | 5 | Not production |
| `src/plans/indexPlan.ts` | Upload | **KEEP** | A | — | |
| `src/plans/resolvePageClassificationForPipeline.ts` | Stage 2 | **KEEP** call from read | B | 2 | |
| `src/plans/buildPlanReadingOrder.ts` | Stage 3 | **KEEP** | B | 2 | |
...
| New `tests/scopes/framing/reset/*` | — | **CREATE** | tests | 1–5 | |

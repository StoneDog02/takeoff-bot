/**
 * Capture a short baseline summary from a reset-takeoff.json after Beckstead.
 * Usage:
 *   npx tsx scripts/summarize-reset-takeoff.ts artifacts/<project>/framing/reset-takeoff.json
 */
import { readFile } from "node:fs/promises";
import path from "node:path";

import { resetTakeoffSchema } from "../src/scopes/framing/reset/resetTakeoff.schema.js";

async function main(): Promise<void> {
  const takeoffPath = process.argv[2];
  if (!takeoffPath) {
    console.error("Usage: npx tsx scripts/summarize-reset-takeoff.ts <reset-takeoff.json>");
    process.exit(1);
  }

  const raw = JSON.parse(await readFile(path.resolve(takeoffPath), "utf8"));
  const takeoff = resetTakeoffSchema.parse(raw);

/**
 * Beckstead reset milestone via frozen Evidence replay (no live Claude).
 *
 * Use when live Anthropic extraction is unavailable; proves:
 * PDF index → interpret → calculate → reset-takeoff.json
 *
 * Usage:
 *   npx tsx scripts/run-beckstead-reset-replay.ts \
 *     [--evidence artifacts/b2.3-wave5/runs/beckstead-wave5-after/framing/06-extractedEvidence.json] \
 *     [--project beckstead-reset-replay]
 */
...
import { RESET_TAKEOFF_FILENAME } from "../src/scopes/framing/reset/writeResetTakeoff.js";
...
async function main(): Promise<void> {
  ...
  const projectId = argValue(argv, "--project", "beckstead-reset-replay");
  ...
  if (!result.success || !result.takeoffPath || !result.takeoff) {
    console.error("Beckstead reset replay failed:");
  ...
  const baseline = {
    ...
    notes: [
      ...
      "This replay proves interpret→calculate→reset-takeoff without Stage 13/15/16.",
      "Zeros by domain are capability/input gaps, not permission-gate failures.",
    ],
  };

  const baselinePath = path.join(baselineDir, "reset-beckstead-baseline.json");
  await writeFile(baselinePath, `${JSON.stringify(baseline, null, 2)}\n`, "utf8");

  console.log(`✓ Beckstead reset replay complete`);
  ...
  console.log(`  File:     ${RESET_TAKEOFF_FILENAME}`);
}

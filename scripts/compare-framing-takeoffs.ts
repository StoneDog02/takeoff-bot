/**
 * Compare two framing-takeoff.json artifacts (e.g. live vs beckstead-reset-m1).
 *
 * Usage:
 *   npx tsx scripts/compare-framing-takeoffs.ts \
 *     artifacts/beckstead-reset-m1/framing/framing-takeoff.json \
 *     artifacts/<live-project>/framing/framing-takeoff.json
 */
...
function loadTakeoff(raw: unknown): FramingTakeoff {
  return framingTakeoffSchema.parse(raw);
}

function byDomain(materials: readonly FramingMaterialLine[]): Record<string, number> {
...
function lineKey(line: FramingMaterialLine): string {
  return [
    line.domain ?? "",
    line.quantityKey ?? "",
    line.description,
...
function summarize(label: string, takeoff: FramingTakeoff) {
  return {
    label,
...
async function main(): Promise<void> {
  ...
  if (!leftPath || !rightPath) {
    console.error(
      "Usage: npx tsx scripts/compare-framing-takeoffs.ts <left.json> <right.json>",
    );
    process.exit(1);
  }
  ...
  console.log("=== Framing takeoff comparison ===");
